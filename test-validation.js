...
async function testValidation() {
  ...
  const emailError = await validateField(
    "invalid-email",
    ValidationRules.email(),
  );
  console.log(`   Invalid email: "${emailError}"`);

  const validEmail = await validateField(
    "test@example.com",
    ValidationRules.email(),
  );
  ...
  const invalidStellar = await validateField(
    "invalid-address",
    ValidationRules.stellarAddress(),
  );
  console.log(`   Invalid Stellar address: "${invalidStellar}"`);

  const validStellar = await validateField(
    "GABCDEFGHIJKLMNOPQRSTUVWXYZ123456789012345678901234567890",
    ValidationRules.stellarAddress(),
  );
  ...
  const negativeNumber = await validateField(
    "-10",
    ValidationRules.positiveNumber(),
  );
  console.log(`   Negative number: "${negativeNumber}"`);

  const validNumber = await validateField(
    "100",
    ValidationRules.positiveNumber(),
  );
