...
use soroban_sdk::{contractimpl, symbol, Address, Env, IntoVal, Storage, Vec, Map, token, TokenIdentifier, BytesN, panic_with_error};
...
impl SkillTreeContract {
    ...
    pub fn admin(env: &Env) -> Address {
        env.storage().get_unchecked::<_, Address>(&symbol!(KEY_ADMIN)).unwrap()
    }
...
mod test {
    ...
    fn test_mint_character_works() {
        ...
        let result = std::panic::catch_unwind(|| contract.mint_character(env.clone(), user.clone()));
    ...
    fn test_mint_badge_works() {
        ...
        let result2 = std::panic::catch_unwind(|| contract.mint_badge(env.clone(), user.clone(), 10));
