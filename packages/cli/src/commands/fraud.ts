  ...
  fraud
    ...
    .action(async (transactionId: string, options) => {
      ...
      const answers = await inquirer.prompt([
        {
          ...
          validate: (value: string) =>
            value ? true : "Account id is required",
        },
        {
          type: "number",
          name: "amount",
      ...
      try {
        ...
        fallback = true;
        spinner.warn(
          "Realtime fraud model unavailable, using heuristic scoring",
        );
        riskScore = Math.min(
          100,
          Math.round(
            transaction.amount / 20 +
              (transaction.channel === "online" ? 25 : 0),
          ),
        );
        if (
          transaction.country &&
          !["US", "CA", "GB", "DE"].includes(transaction.country.toUpperCase())
        ) {
          riskScore += 10;
        }
        if (transaction.ipAddress) {
          reasons.push(
            `IP flagged for manual review (${transaction.ipAddress})`,
          );
        }
        ...
        if (!reasons.length) {
          reasons.push(
            "Limited telemetry available, defaulting to manual follow-up",
          );
        }
