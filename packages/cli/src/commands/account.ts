...
export function registerAccountCommand(program: Command): void {
  const account = program
    .command("account")
    .description("Manage ChenAIKit blockchain accounts");
  ...
  account
    ...
    .action(async (options) => {
      ...
      const network = resolveNetwork(
        options.network ?? prompts.network,
        config.network,
      );
      const saveSecret: boolean = Boolean(
        options.saveSecret ?? prompts.saveSecret,
      );
      const setDefault: boolean = Boolean(
        options.setDefault ?? prompts.setDefault ?? !config.defaultAccount,
      );
      ...
      try {
        ...
        console.log(
          `${chalk.gray("Public Key:")} ${chalk.cyan(profile.publicKey)}`,
        );
        if (saveSecret) {
          console.log(
            `${chalk.gray("Secret Key:")} Stored securely in config (remember to protect the file!)`,
          );
        } else {
          console.log(
            `${chalk.gray("Secret Key:")} ${chalk.yellow(keypair.secret())}`,
          );
          console.log(
            chalk.red(
              "Make sure to copy this secret key now. It will not be shown again.",
            ),
          );
        }
        if (setDefault) {
    ...
    .action(async (address: string | undefined, options) => {
      ...
      try {
        ...
        if (!balances.length) {
          console.log(
            chalk.yellow(
              "No balances reported by Horizon. The account might be unfunded.",
            ),
          );
          return;
        }
        ...
        console.error(
          chalk.red(
            error instanceof Error
              ? error.message
              : "An unexpected error occurred while fetching the account balance.",
          ),
        );
        ...
        if (profile) {
          ...
          console.log(
            chalk.gray("Network access failed, showing cached metadata only."),
          );
        }
        process.exitCode = 1;
      }
