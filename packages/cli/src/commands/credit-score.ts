...
export function registerCreditScoreCommand(program: Command): void {
  const creditScore = program
    ...
    .description("AI-powered credit scoring tools");
  ...
  creditScore
    ...
    .option(
      "-n, --network <network>",
      "Specify network used for telemetry (optional)",
    )
    .action(async (address: string | undefined, options) => {
        ...
        const answer = await inquirer.prompt([
          {
            ...
            validate: (value: string) =>
              value.startsWith("G")
                ? true
                : "Enter a Stellar public key (starts with G).",
          },
        ]);
      ...
      const profileAnswers = await inquirer.prompt([
        {
          ...
          validate: (value: number) =>
            value > 0 ? true : "Income must be positive",
        },
        {
          type: "number",
          name: "monthlyDebt",
          ...
          default: 1500,
          validate: (value: number) =>
            value >= 0 ? true : "Debt cannot be negative",
        },
        {
          type: "number",
          name: "historyYears",
          ...
          default: 3,
          validate: (value: number) =>
            value >= 0 ? true : "History cannot be negative",
        },
        {
          type: "number",
          name: "onTimePayments",
          ...
          default: 18,
          validate: (value: number) =>
            value >= 0 ? true : "Value cannot be negative",
        },
        {
          type: "number",
      ...
      console.log(
        `${chalk.gray("Monthly DTI:")} ${((inputs.monthlyDebt / inputs.monthlyIncome) * 100).toFixed(1)}%`,
      );
