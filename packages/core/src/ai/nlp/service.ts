...
export class NLPService {
  ...
  async processMessage(
    ...
    const context =
      (await this.contextStore.get(conversationId)) ||
      this.createContext(conversationId);
    ...
    context.pendingIntent = classification.missingEntities.length
    ...
    context.metadata = {
      ...
      awaitingConfirmation:
        Boolean(operation) &&
        (classification.intent === "send_payment" ||
          classification.intent === "swap_asset"),
    };
  ...
  private parseModelClassification(
    raw: string,
    userInput: string,
  ): IntentClassification {
    ...
    try {
      ...
      const intent: WalletIntent = allowed.includes(
      ...
      const confidence = Math.max(
        0,
        Math.min(1, parsed.confidence || fallback.confidence),
      );
      ...
      return {
        ...
        requiresConfirmation:
          intent === "send_payment" || intent === "swap_asset",
      };
    } catch {
      return fallback;
    }
  ...
  private applyMultiTurnContext(
    ...
    if (
      ...
      merged.missingEntities = inferMissingEntities(
        merged.intent,
        merged.entities,
      );
      merged.requiresConfirmation =
        merged.intent === "send_payment" || merged.intent === "swap_asset";
      return merged;
    }

    if (
      context.pendingIntent &&
      classification.intent === context.pendingIntent
    ) {
      classification.missingEntities = inferMissingEntities(
        classification.intent,
  ...
  private composeReply(
    ...
    if (classification.missingEntities.length > 0) {
      return followUpQuestion(
        classification.intent,
        classification.missingEntities,
      );
    }
