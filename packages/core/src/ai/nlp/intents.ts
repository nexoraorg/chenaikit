...
> = {
  ...
  send_payment:
    "Send a blockchain payment after validating amount and address.",
...
export function classifyIntentRuleBased(input: string): IntentClassification {
  ...
  const confidence =
    bestIntent === "unknown" ? 0.3 : Math.min(0.95, 0.5 + bestScore * 0.2);
  ...
  return {
    ...
    requiresConfirmation:
      bestIntent === "send_payment" || bestIntent === "swap_asset",
  };
}
