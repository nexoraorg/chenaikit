  ...
  recognize(
    ...
    if (burst > 0)
      out.push({
        name: "burst_activity",
        score: burst,
        reason: `High transaction velocity: ${vel1}/1m, ${vel5}/5m`,
      });
    ...
    if (geo > 0)
      out.push({
        ...
        score: geo,
        reason: `Geo velocity ${kmph.toFixed(1)} km/h`,
      });
    ...
    if (novelty > 0.7)
      out.push({
        name: "merchant_novelty",
        score: novelty,
        reason: "Unseen merchant for account",
      });
    ...
    if (countryRisk > 0)
      out.push({
        ...
        score: 0.8,
        reason: `Transaction in high-risk country: ${tx.country}`,
      });
    ...
    if (hourDev > 0.8)
      out.push({
        name: "odd_hour",
        score: hourDev,
        reason: "Unusual transaction hour for user",
      });
    ...
    if (struct > 0)
      out.push({
        name: "amount_structuring",
        score: struct,
        reason: "Multiple small transfers indicative of structuring",
      });
    ...
    if (chRisk > 0.9)
      out.push({
        ...
        score: 0.5,
        reason: `Channel ${tx.channel} has elevated risk`,
      });
