...
export class FeatureExtractor {
  ...
  private updateHistory(tx: Transaction) {
    ...
    if (tx.merchantId)
      h.merchantCounts.set(
        tx.merchantId,
        (h.merchantCounts.get(tx.merchantId) || 0) + 1,
      );
    const hour = new Date(tx.timestamp).getHours();
    h.hourHistogram[hour]++;
    // Trim history
    const max = this.opts.maxHistoryPerAccount;
    if (h.txTimestamps.length > max)
      h.txTimestamps.splice(0, h.txTimestamps.length - max);
    ...
    if (h.merchantCounts.size > max) {
      ...
      for (const key of h.merchantCounts.keys()) {
        h.merchantCounts.set(
          key,
          Math.max(0, Math.floor((h.merchantCounts.get(key) || 0) * 0.9)),
        );
      }
    }
    // hour histogram decay
    for (let i = 0; i < 24; i++)
      h.hourHistogram[i] = Math.floor(h.hourHistogram[i] * 0.995);
  }
  ...
  extract(tx: Transaction): FeatureVector {
    ...
    // Merchant novelty
    const merchantFreq = tx.merchantId
      ? h.merchantCounts.get(tx.merchantId) || 0
      : 0;
    ...
    const hours = h.lastTx
      ? Math.max(1e-6, (tx.timestamp - h.lastTx.timestamp) / (3600 * 1000))
      : 0;
    ...
    const deviceChanged =
      h.lastTx?.deviceId && tx.deviceId && h.lastTx.deviceId !== tx.deviceId
        ? 1
        : 0;
    ...
    const countryRisk =
      tx.country && HIGH_RISK_COUNTRIES.has(tx.country) ? 1 : 0;
    names.push("country_risk");
    values.push(countryRisk);

    // Balance impact ratio
    const balanceImpact = tx.previousBalance
      ? tx.amount / Math.max(1, tx.previousBalance)
      : 0;
    ...
    if (elapsed > 50) {
      ...
      hist.txTimestamps = hist.txTimestamps.slice(
        -Math.floor(this.opts.maxHistoryPerAccount / 2),
      );
      hist.amounts = hist.amounts.slice(
        -Math.floor(this.opts.maxHistoryPerAccount / 2),
      );
    }
