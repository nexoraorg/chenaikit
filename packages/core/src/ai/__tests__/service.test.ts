...
describe("AIService", () => {
  ...
  describe("calculateCreditScore", () => {
    it("should throw error as not implemented", async () => {
      ...
      await expect(aiService.calculateCreditScore(accountData)).rejects.toThrow(
        "Not implemented yet - see issue #25",
      );
    });
  });
  ...
  describe("detectFraud", () => {
    it("should throw error as not implemented", async () => {
      ...
      await expect(aiService.detectFraud(transactionData)).rejects.toThrow(
        "Not implemented yet - see issue #28",
      );
    });
  });
});
