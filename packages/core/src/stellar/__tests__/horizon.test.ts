...
describe("HorizonConnector", () => {
  ...
  describe("Account Operations", () => {
    ...
    it("should throw error for invalid address format", async () => {
      await expect(horizon.getAccount(TEST_ACCOUNTS.INVALID)).rejects.toThrow(
    ...
    it("should throw error for non-existent account", async () => {
      ...
      await expect(horizon.getAccount(nonExistentAccount)).rejects.toThrow(
        "Account not found",
      );
    });
  });
  ...
  describe("Transaction History", () => {
    ...
    it("should handle pagination correctly", async () => {
      const firstPage = await horizon.getAccountTransactions(
        TEST_ACCOUNTS.SDF,
        {
          limit: 2,
          order: "desc",
      ...
      if (firstPage.next) {
        ...
        const nextPage = await horizon.getAccountTransactions(
          ...
          {
            cursor:
              firstPage.records[firstPage.records.length - 1].paging_token,
            limit: 2,
            order: "desc",
          },
        );
  ...
  describe("Transaction Details", () => {
    ...
    it("should fetch transaction operations", async () => {
      ...
      const result =
        await horizon.getTransactionOperations(testTransactionHash);
    ...
    it("should throw error for invalid transaction hash", async () => {
      await expect(horizon.getTransaction("invalid_hash")).rejects.toThrow(
    ...
    it("should throw error for non-existent transaction", async () => {
      ...
      await expect(horizon.getTransaction(nonExistentHash)).rejects.toThrow(
  ...
  describe("Ledger Operations", () => {
    ...
    it("should fetch specific ledger by sequence", async () => {
      ...
      const recentLedgers = await horizon.getLedgers({
        limit: 1,
        order: "desc",
      });
    ...
    it("should throw error for non-existent ledger", async () => {
      await expect(horizon.getLedger(999999999)).rejects.toThrow(
        "Ledger not found",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle network errors gracefully", async () => {
      ...
      await expect(invalidHorizon.healthCheck()).resolves.toBe(false);
    });
    ...
    it("should handle rate limiting", async () => {
      ...
      const promises = Array(3)
        .fill(null)
        .map(() => rateLimitedHorizon.getNetworkInfo());
