...
describe("AssetManager", () => {
  ...
  beforeEach(() => {
    mockConnector = new StellarConnector({
      network: "testnet",
    }) as jest.Mocked<StellarConnector>;
    mockAsset = new Asset(
      "USD",
      "GTEST123456789012345678901234567890123456789",
    );
    ...
    mockConnector.getNetworkPassphrase = jest
      .fn()
      .mockResolvedValue("Test SDF Network ; September 2015");
    mockConnector.submitTransaction = jest
      .fn()
      .mockResolvedValue({ hash: "test-hash", success: true });
  });

  describe("createTrustline", () => {
    it("should create a trustline with default limit", async () => {
      const sourceSecret =
        "S1234567890123456789012345678901234567890123456789012345678901234";
    ...
    it("should handle errors when creating trustline", async () => {
      const sourceSecret =
        "S1234567890123456789012345678901234567890123456789012345678901234";
      mockConnector.submitTransaction.mockRejectedValue(
        new Error("Transaction failed"),
      );

      await expect(
        createTrustline(mockConnector, sourceSecret, mockAsset),
  ...
  describe("issueAsset", () => {
    it("should issue an asset to distribution account", async () => {
      const issuerSecret =
        "S1234567890123456789012345678901234567890123456789012345678901234";
      const distributionPublicKey =
        "GDIST123456789012345678901234567890123456789";
      const amount = "1000";
  ...
  describe("transferAsset", () => {
    it("should transfer asset between accounts", async () => {
      ...
      const destinationPublicKey =
        "GDEST123456789012345678901234567890123456789";
      const amount = "500";
