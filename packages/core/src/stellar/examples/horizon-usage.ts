...
async function demonstrateHorizonUsage() {
  ...
  try {
    ...
    ledgers.records.forEach((ledger, index) => {
      ...
      console.log(
        `     Closed: ${new Date(ledger.closed_at).toLocaleString()}`,
      );
    });
    ...
    try {
      ...
      console.log(
        "✓ Correctly caught invalid address error:",
    ...
    try {
      ...
      console.log(
        "✓ Correctly caught non-existent account error:",
        (error as Error).message,
      );
    }
...
async function demonstrateStreaming() {
  ...
  const streamPromise = horizon.streamAccount(testAccount, (accountData) => {
    ...
    console.log(
      `   Last Modified: ${new Date(accountData.last_modified_time).toLocaleString()}`,
    );
  });
