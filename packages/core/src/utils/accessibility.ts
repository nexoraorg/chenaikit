...
export function generatePerformanceDescription(
...
export function generateUserActivityDescription(
...
export function createAccessibleDataTable(
  ...
  const caption =
    title ||
    `Data table with ${data.length} rows and ${headers.length} columns`;
