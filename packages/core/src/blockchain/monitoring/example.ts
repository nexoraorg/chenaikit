...
class MonitoringApplication {
  ...
  private setupEventHandlers(): void {
    ...
    this.monitor.on("metrics", (metrics: any) => {
      console.log(
        `📈 Batch metrics - TPS: ${metrics.transactionsPerSecond.toFixed(2)}, Volume: ${metrics.totalVolume.toFixed(2)}`,
      );
    });
  }
  ...
  private handleTransaction(
    ...
    console.log(
      `${emoji} Transaction: ${transaction.hash.substring(0, 12)}... | ${riskLevel} | Account: ${transaction.sourceAccount.substring(0, 8)}...`,
    );

    // Log high-risk transactions with details
    if (analysis.riskScore > 50) {
      console.log(
        `   🔍 Analysis: Category=${analysis.category}, Risk=${analysis.riskScore}, Flags=[${analysis.flags.join(", ")}]`,
      );
    }
  }
  ...
  private setupCustomAlertRules(): void {
    ...
    const whaleRule: AlertRule = {
      ...
      description:
        "Detect movements of very large amounts (whale transactions)",
      conditions: [
        {
  ...
  private async demonstrateDashboard(): Promise<void> {
    ...
    console.log(
      `      - Total Transactions: ${dashboardData.overview.realTimeMetrics.totalTransactions}`,
    );
    console.log(
      `      - Success Rate: ${((dashboardData.overview.realTimeMetrics.successfulTransactions / dashboardData.overview.realTimeMetrics.totalTransactions) * 100 || 0).toFixed(1)}%`,
    );
    console.log(
      `      - TPS: ${dashboardData.overview.realTimeMetrics.transactionsPerSecond.toFixed(2)}`,
    );
    console.log(
      `      - Total Volume: ${dashboardData.overview.realTimeMetrics.totalVolume.toFixed(2)}`,
    );

    console.log(`   🚨 Alert Summary:`);
    const alertSummary = (dashboardData.overview as any).alertSummary || {
      total: 0,
      unacknowledged: 0,
    ...
    console.log(
      `   ⚡ System Health: ${dashboardData.overview.systemHealth.status.toUpperCase()}`,
    );
  }
  ...
  private async demonstrateAnalytics(): Promise<void> {
    ...
    console.log(
      `      - Average Value: ${realtimeMetrics.averageTransactionValue.toFixed(2)}`,
    );
  ...
  private async demonstrateSystemHealth(): Promise<void> {
    ...
    console.log(
      `      - Last Connected: ${connectionStatus.lastConnected?.toISOString() || "Never"}`,
    );
  }
  ...
  private async demonstrateTransactionReplay(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `      - Use: monitor.replayTransactions(startLedger, endLedger)`,
      );
      console.log(`      - Use: monitor.verifyTransaction(transactionHash)`);
    } catch (error) {
      console.log(
        `   ℹ️  Replay demo skipped (requires specific ledger range)`,
      );
    }
  }
