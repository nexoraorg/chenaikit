  ...
  try {
    ...
    balances.forEach((balance, index) => {
      if (balance.asset_type === "native") {
        ...
        console.log(
          `      ${index + 1}. ${balance.asset_code}@${balance.asset_issuer}: ${balance.balance}`,
        );
      }
    });
    console.log("");
    ...
    console.log(
      `   ✅ Found ${transactions.records.length} recent transactions:`,
    );
    transactions.records.forEach((tx, index) => {
      console.log(`      ${index + 1}. Hash: ${tx.hash.substring(0, 16)}...`);
      console.log(
        `         Success: ${tx.successful}, Operations: ${tx.operation_count}`,
      );
    });
    console.log("");
    ...
    payments.records.forEach((payment, index) => {
      ...
      console.log(
        `         Amount: ${payment.amount} ${payment.asset_type === "native" ? "XLM" : payment.asset_code}`,
      );
    });
    console.log("");
    ...
    try {
      await horizon.getAccount(
        "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF",
      );
    } catch (error) {
      console.log(
        `   ✅ Correctly caught non-existent account: ${error.message}`,
      );
    }
    console.log("");

    console.log(
      "🎉 All tests passed! Horizon API wrapper is working correctly.",
    );
    console.log("\n📋 Summary:");
