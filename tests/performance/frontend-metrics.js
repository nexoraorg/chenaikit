...
class FrontendPerformanceTester {
  ...
  async runTest(config) {
    ...
    try {
      ...
      await this.page.emulateNetworkConditions({
        offline: false,
        downloadThroughput: this.getNetworkSpeed(config.networkThrottling)
          .download,
  ...
  calculateCoverage(coverage) {
    ...
    const usedBytes = coverage.reduce((sum, entry) => {
      return (
        sum +
        entry.ranges.reduce(
          (rangeSum, range) => rangeSum + (range.end - range.start),
          0,
        )
      );
    }, 0);
  ...
  printTestResult(result) {
    ...
    console.log(
      `   JS Coverage: ${result.jsCoverage.coveragePercentage.toFixed(1)}%`,
    );
    console.log(
      `   CSS Coverage: ${result.cssCoverage.coveragePercentage.toFixed(1)}%`,
    );
    console.log();
  }
  ...
  generateSummary() {
    ...
    const averageJSCoverage =
      this.results
        .filter((r) => r.jsCoverage)
        .reduce((sum, r) => sum + r.jsCoverage.coveragePercentage, 0) /
      (totalTests || 1);

    const averageCSSCoverage =
      this.results
        .filter((r) => r.cssCoverage)
        .reduce((sum, r) => sum + r.cssCoverage.coveragePercentage, 0) /
      (totalTests || 1);
  ...
  generateRecommendations() {
    ...
    // Analyze coverage
    const lowJSCoverage = this.results.filter(
      (r) => r.jsCoverage && r.jsCoverage.coveragePercentage < 50,
    );
    ...
    // Analyze CLS
    const highCLS = this.results.filter(
      (r) => r.metrics?.webVitals?.cumulativeLayoutShift > 0.1,
    );
    if (highCLS.length > 0) {
      recommendations.push({
        type: "ux",
  ...
  printSummary(summary) {
    ...
    if (summary.failedTests > 0) {
      console.log(
        "⚠️  Some tests failed. See detailed report for recommendations.",
      );
    } else {
      console.log(
        "🎉 All tests passed! Performance is within acceptable thresholds.",
      );
    }
  }
    ...
    for (const config of TEST_CONFIGURATIONS) {
      try {
        ...
        const result = await this.runLighthouseAudit(config.url, {
          settings: {
            ...
            throttling: {
              ...
              throughputKbps:
                (this.getNetworkSpeed(config.networkThrottling).download * 8) /
                1024,
        ...
        const reportPath = path.join(
          __dirname,
          `lighthouse-${config.name.toLowerCase().replace(/\s+/g, "-")}.html`,
        );
        fs.writeFileSync(reportPath, result.report);
        console.log(`   Report saved: ${reportPath}`);
      } catch (error) {
        console.error(
          `❌ Lighthouse audit failed for ${config.name}:`,
          error.message,
        );
      }
    }
