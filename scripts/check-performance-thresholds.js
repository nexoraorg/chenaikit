...
function checkApiPerformance(results) {
  ...
  if (
    ...
    metrics.http_req_duration.avg > THRESHOLDS.api.avgResponseTime
  ) {
    issues.push({
  ...
  if (
    metrics.http_req_duration &&
    metrics.http_req_duration["p(95)"] > THRESHOLDS.api.p95ResponseTime
  ) {
    issues.push({
  ...
  if (
    ...
    metrics.http_req_failed.rate > THRESHOLDS.api.errorRate
  ) {
    issues.push({
      type: "errorRate",
  ...
  if (issues.length === 0) {
    ...
    issues.forEach((issue) => {
      ...
      log(
        color,
        `   ${issue.type}: ${issue.actual.toFixed(2)} (threshold: ${issue.threshold})`,
      );
    });
  }
...
function checkFrontendPerformance(results) {
  ...
  if (
    results.lighthouse &&
    results.lighthouse.categories &&
    results.lighthouse.categories.performance
  ) {
  ...
  if (
    results.bundleSize &&
    results.bundleSize > THRESHOLDS.frontend.bundleSize
  ) {
    issues.push({
      type: "bundleSize",
  ...
  if (issues.length === 0) {
    ...
    issues.forEach((issue) => {
      ...
      log(
        color,
        `   ${issue.type}: ${issue.actual} (threshold: ${issue.threshold})`,
      );
    });
  }
...
function checkDatabasePerformance(results) {
  ...
  if (
    ...
    results.avgQueryTime > THRESHOLDS.database.avgQueryTime
  ) {
    issues.push({
      type: "avgQueryTime",
  ...
  if (
    ...
    results.p95QueryTime > THRESHOLDS.database.p95QueryTime
  ) {
    issues.push({
      type: "p95QueryTime",
  ...
  if (
    results.indexUsage &&
    results.indexUsage < THRESHOLDS.database.indexUsage
  ) {
    issues.push({
      type: "indexUsage",
  ...
  if (
    ...
    results.cacheHitRate < THRESHOLDS.database.cacheHitRate
  ) {
    issues.push({
      type: "cacheHitRate",
...
function checkContractPerformance(results) {
  ...
  if (
    results.avgGasUsage &&
    results.avgGasUsage > THRESHOLDS.contracts.avgGasUsage
  ) {
    issues.push({
      type: "avgGasUsage",
  ...
  if (
    results.maxGasUsage &&
    results.maxGasUsage > THRESHOLDS.contracts.maxGasUsage
  ) {
    issues.push({
      type: "maxGasUsage",
  ...
  if (
    ...
    results.avgExecutionTime > THRESHOLDS.contracts.avgExecutionTime
  ) {
    issues.push({
