...
describe("FeatureFlagService", () => {
  ...
  describe("evaluateFlag", () => {
    ...
    it("should respect user-specific overrides", () => {
      ...
      const result = featureFlagService.evaluateFlag("user_flag", {
        userId: "user-123",
      });
      expect(result.value).toBe(true);
      expect(result.source).toBe("override");

      const otherResult = featureFlagService.evaluateFlag("user_flag", {
        userId: "other-user",
      });
    ...
    it("should evaluate targeting by user", () => {
      ...
      const result = featureFlagService.evaluateFlag("targeted_flag", {
        userId: "premium-user",
      });
      expect(result.value).toBe(true);
      expect(result.source).toBe("targeting");

      const otherResult = featureFlagService.evaluateFlag("targeted_flag", {
        userId: "free-user",
      });
    ...
    it("should evaluate targeting by segment", () => {
      ...
      const result = featureFlagService.evaluateFlag("segment_flag", {
        segments: ["beta"],
      });
      expect(result.value).toBe(true);
      expect(result.source).toBe("targeting");
    ...
    it("should evaluate targeting by property", () => {
      featureFlagService.createFlag({
        ...
        defaultValue: false,
        targeting: [
          { type: "property", values: ["mobile"], property: "platform" },
        ],
      });
    ...
    it("should respect rollout percentage", () => {
      ...
      const result = featureFlagService.evaluateFlag("rollout_flag", {
        userId: "any-user",
      });
      expect(result.value).toBe(false);
  ...
  describe("getAuditLog", () => {
    it("should log create actions", () => {
      ...
      const createEntry = log.find(
        (e) => e.flagKey === "audited_flag" && e.action === "create",
      );
    ...
    it("should log toggle actions", () => {
      ...
      const log = featureFlagService.getAuditLog();
      const toggleEntry = log.find(
        (e) => e.flagKey === "toggle_audit" && e.action === "toggle",
      );
