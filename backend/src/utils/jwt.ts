...
const getSecret = (
  ...
  if (
    ...
    return key === "ACCESS_TOKEN_SECRET"
      ? "dev_access_secret"
      : "dev_refresh_secret";
  }

  throw new Error(
    `${key} is required (missing for NODE_ENV=${process.env.NODE_ENV || "unknown"})`,
  );
};
