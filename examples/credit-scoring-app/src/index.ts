...
export function calculateCreditScore(accountData: any): number {
  ...
  const avgTransactionAmount =
    transactionHistory.reduce(
      ...
      0,
    ) / Math.max(transactionHistory.length, 1);
...
class CreditScoringMonitor {
  ...
  public processTransaction(
    ...
    const currentScore =
      this.accountScores.get(transaction.sourceAccount) || 600;
    ...
    console.log(
      `📊 Account ${transaction.sourceAccount.substring(0, 8)}...: Score ${newScore} (Risk: ${analysis.riskScore.toFixed(1)})`,
    );
  }
  ...
  private triggerFraudDetection(alert: Alert): void {
    console.log(
      `🔍 Triggering enhanced fraud detection for alert: ${alert.id}`,
    );
  ...
  getMetrics() {
    return {
      ...
      averageScore:
        Array.from(this.accountScores.values()).reduce(
          (sum, score) => sum + score,
          0,
        ) / Math.max(this.accountScores.size, 1),
      recentAlerts: this.riskAlerts.length,
      highRiskAccounts: Array.from(this.accountScores.values()).filter(
        (score) => score < 500,
      ).length,
    };
  }
