...
const LiveTransactionFeed: React.FC<LiveTransactionFeedProps> = ({
  ...
  useEffect(() => {
    if (!isPaused) {
      setTransactions((prev: any[]) => {
        const newTransactions = recentTransactions
          .slice(0, maxItems - prev.length)
          .map((tx: any) => ({
            transaction: tx,
            analysis: {
              transactionId: tx.id,
        ...
        return [
          ...newTransactions,
          ...prev.slice(0, maxItems - newTransactions.length),
        ];
      });
    }
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {transactions.map(
            ({ transaction, analysis, timestamp }, index: number) => (
              <div
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <span
                      className={`text-xs px-2 py-1 rounded ${getCategoryColor(analysis.category)}`}
                    >
