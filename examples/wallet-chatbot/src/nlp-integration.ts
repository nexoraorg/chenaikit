...
export class WalletChatbotNLPIntegration {
  ...
  async handleUserMessage(
    ...
    const result = await this.nlpService.processMessage(
      conversationId,
      message,
    );
    ...
    if (
      (operation.type === "send_payment" || operation.type === "swap_asset") &&
      !isConfirmed
    ) {
    ...
    if (operation.type === "get_transactions") {
      return this.handlers.listTransactions(
        String(operation.params.timeframe || "recent"),
      );
    }
...
export const trainingData: TrainingExample[] = [
  ...
  { intent: "check_balance", utterance: "How much XLM do I have right now?" },
  {
    intent: "send_payment",
    utterance:
      "Send 25 XLM to GABCD1234567890ABCDEFGHJKLMNPQRSTUVWXYZABCD12345",
    entities: [
      { type: "amount", value: "25" },
      { type: "asset", value: "XLM" },
      {
        type: "address",
        value: "GABCD1234567890ABCDEFGHJKLMNPQRSTUVWXYZABCD12345",
      },
    ],
  },
...
export const exampleConversations: ExampleConversation[] = [
  {
    ...
    turns: [
      {
        user: "Can you check my wallet balance?",
        assistant: "I can fetch your current wallet balance now.",
      },
    ...
    turns: [
      {
        ...
        assistant:
          "I can help with that payment. What amount should I send and to which wallet address?",
      },
      {
        user: "Send 30 USDC",
        assistant:
          "Please share the destination wallet address before I prepare the transaction.",
      },
      {
        user: "Send to GABCD1234567890ABCDEFGHJKLMNPQRSTUVWXYZABCD12345",
        assistant:
          "I can prepare a transfer of 30 USDC to GABCD1234567890ABCDEFGHJKLMNPQRSTUVWXYZABCD12345. Please confirm before I execute anything on-chain.",
      },
    ],
  },
];
