...
> = ({
  ...
  const processedData = React.useMemo(() => {
    if (chartType === "radar") {
      ...
      const modelData = data.reduce(
        (acc: Record<string, any>, item: PerformanceMetrics) => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const tooltipContent = generateTooltipContent([
        {
          label: "Model",
          value: data.modelName || "Unknown",
          color: CHART_COLORS.primary[0],
        },
        {
          label: "Accuracy",
          value: formatNumber(data.accuracy * 100, 2) + "%",
          color: CHART_COLORS.primary[1],
        },
        {
          label: "Precision",
          value: formatNumber(data.precision * 100, 2) + "%",
          color: CHART_COLORS.primary[2],
        },
        {
          label: "Recall",
          value: formatNumber(data.recall * 100, 2) + "%",
          color: CHART_COLORS.primary[3],
        },
        {
          label: "F1 Score",
          value: formatNumber(data.f1Score * 100, 2) + "%",
          color: CHART_COLORS.primary[4],
        },
        {
          label: "Date",
          value: data.fullDate || data.date,
        ...
        {payload.map((entry: any, index: number) => (
          <div
            ...
            style={{
              ...
              cursor: "pointer",
              opacity:
                selectedMetric && selectedMetric !== entry.dataKey ? 0.5 : 1,
              transition: "opacity 0.2s",
            }}
            onClick={() => {
              const newSelection =
                selectedMetric === entry.dataKey ? null : entry.dataKey;
  ...
  return (
    <div
      ...
      id={id}
      aria-label={
        ariaLabel || getAriaLabel(processedData, "Performance Metrics")
      }
      aria-describedby={ariaDescribedby}
    >
