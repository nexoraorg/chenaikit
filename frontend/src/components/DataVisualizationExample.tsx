...
const generateSampleData = () => {
  ...
  const transactions: TransactionData[] = Array.from(
    { length: 50 },
    (_, i) => ({
  ...
  const performanceData: PerformanceMetrics[] = Array.from(
    { length: 30 },
    (_, i) => ({
  ...
  const userActivity: UserActivity[] = Array.from({ length: 200 }, () => ({
    ...
    action: ["login", "transaction", "view", "click", "scroll"][
      Math.floor(Math.random() * 5)
    ],
    value: Math.random() * 100,
    category: ["authentication", "trading", "navigation", "engagement"][
      Math.floor(Math.random() * 4)
    ],
    location:
      Math.random() > 0.5
        ? `Location_${Math.floor(Math.random() * 10)}`
        : undefined,
  }));
...
export const DataVisualizationExample: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "flow" | "performance" | "heatmap" | "network"
  >("flow");
  ...
  const handleExportTransactions = useCallback(() => {
    exportTransactionData(data.transactions, {
      format: "csv",
  ...
  const handleExportPerformance = useCallback(() => {
    exportPerformanceData(data.performanceData, {
      format: "csv",
  ...
  const handleExportUserActivity = useCallback(() => {
    exportUserActivityData(data.userActivity, {
      format: "csv",
      filename: "user-activity",
    });
  }, [data.userActivity]);
  ...
  const handleExportNetwork = useCallback(() => {
    exportNetworkData(data.nodes, data.links, {
      format: "csv",
      filename: "network",
    });
    ...
    >
      <div style={{ marginBottom: "20px" }}>
        ...
        <p style={{ color: "#6B7280", marginBottom: "20px" }}>
          Interactive data visualization components for blockchain and AI
          applications
        </p>
