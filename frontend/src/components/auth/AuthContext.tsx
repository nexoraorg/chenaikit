...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setSession = useCallback((newAccessToken: string, newRefreshToken: string, rememberMe: boolean) => {
  ...
  const register = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || 'Registration failed. Please check the entered data.';
      setError(errMsg);
      throw new Error(errMsg);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      const hasRefreshToken = !!(localStorage.getItem(REFRESH_TOKEN_KEY) || sessionStorage.getItem(REFRESH_TOKEN_KEY));
      if (hasRefreshToken) {
        try {
