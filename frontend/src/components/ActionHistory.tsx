...
import type { ActionType, UndoRedoAction } from '../contexts/UndoRedoContext';
...
export const ActionHistory: React.FC<ActionHistoryProps> = ({
  ...
  const { undoStack, redoStack, jumpTo, clear, restoreActions } = useUndoRedo();
        ...
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          {persistedCount > 0 && undoStack.length === 0 && (
            <Tooltip title={`${persistedCount} actions from previous session`}>
              <IconButton size="small" color="default" aria-label="Previous session actions">
          ...
          {hasHistory && showClear && (
            <Tooltip title="Clear history">
              <IconButton size="small" onClick={clear} aria-label="Clear history">
                  ...
                  secondaryAction={
                    <Tooltip title="Undo to this point">
                      <IconButton
                        ...
                        aria-label={`Undo to ${action.description}`}
                      >
