...
export const UserActivityHeatmap: React.FC<UserActivityHeatmapProps> = ({
  ...
  const categories = React.useMemo(() => {
    const uniqueCategories = Array.from(
      new Set(processedData.cells.map((cell) => cell.category)),
    );
  ...
  const times = React.useMemo(() => {
    const uniqueTimes = Array.from(
      new Set(processedData.cells.map((cell) => cell.time)),
    );
  ...
  useEffect(() => {
    ...
    const cells = g
      ...
      .on("click", (event, d) => {
        ...
        setSelectedCell(
          selectedCell === `${d.time}-${d.category}`
            ? null
            : `${d.time}-${d.category}`,
        );
        onCellClick?.(d);
      ...
      .on("mouseover", (event, d) => {
        if (showTooltip) {
          const tooltipContent = generateTooltipContent([
            ...
            {
              label: "Intensity",
              value: formatNumber(d.intensity * 100, 1) + "%",
              color: "#8B5CF6",
            },
          ]);
  ...
  return (
    ...
    >
      <svg
        ...
        aria-label={
          ariaLabel ||
          getAriaLabel(processedData.cells, "User Activity Heatmap")
        }
        aria-describedby={ariaDescribedby}
        style={{ display: "block" }}
      />
