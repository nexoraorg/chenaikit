...
export const RiskFactorsList: React.FC<RiskFactorsListProps> = ({
  ...
  const displayedFactors =
    maxItems && !expanded ? factors.slice(0, maxItems) : factors;
      ...
      <CardContent sx={{ p: 3 }}>
        ...
        <List sx={{ p: 0 }}>
          {displayedFactors.map((factor, index) => {
            const impactColor = getSeverityColor(
              factor.impact,
              factor.severity,
            );
        ...
        >
          <Typography
            ...
            sx={{ display: "block" }}
          >
            Risk factors are identified using AI-powered analysis of your
            account activity, transaction patterns, and payment history.
          </Typography>
        </Box>
      </CardContent>
    </Card>
