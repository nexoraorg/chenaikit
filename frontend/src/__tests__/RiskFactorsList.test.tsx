...
describe("RiskFactorsList", () => {
  ...
  describe("Loading State", () => {
    it("renders loading spinner when loading is true", () => {
      renderWithTheme(<RiskFactorsList factors={[]} loading={true} />);
  ...
  describe("Error State", () => {
    it("renders error message when error prop is provided", () => {
      ...
      renderWithTheme(<RiskFactorsList factors={[]} error={errorMessage} />);
  ...
  describe("Empty State", () => {
    it("renders success message when no factors are present", () => {
      renderWithTheme(<RiskFactorsList factors={[]} />);

      expect(
        screen.getByText(/no risk factors identified/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/your credit profile looks great/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Factors Display", () => {
    it("renders list title", () => {
      renderWithTheme(<RiskFactorsList factors={mockFactors} />);
    ...
    it("shows correct count of positive factors", () => {
      ...
      const positiveCount = mockFactors.filter(
        (f) => f.impact === "positive",
      ).length;
    ...
    it("shows correct count of negative factors", () => {
      ...
      const negativeCount = mockFactors.filter(
        (f) => f.impact === "negative",
      ).length;
    ...
    it("displays impact labels correctly", () => {
      ...
      const positiveFactorsCount = mockFactors.filter(
        (f) => f.impact === "positive",
      ).length;
      const negativeFactorsCount = mockFactors.filter(
        (f) => f.impact === "negative",
      ).length;

      expect(positiveLabels.length).toBeGreaterThanOrEqual(
        positiveFactorsCount,
      );
      expect(negativeLabels.length).toBeGreaterThanOrEqual(
        negativeFactorsCount,
      );
    });
  ...
  describe("Max Items Feature", () => {
    it("limits displayed items when maxItems is set", () => {
      renderWithTheme(<RiskFactorsList factors={mockFactors} maxItems={2} />);
    ...
    it("does not show expand/collapse button when all items fit", () => {
      renderWithTheme(<RiskFactorsList factors={mockFactors} maxItems={10} />);
  ...
  describe("Edge Cases", () => {
    it("handles only positive factors", () => {
      ...
      renderWithTheme(<RiskFactorsList factors={positiveFactors} />);
    ...
    it("handles only negative factors", () => {
      ...
      renderWithTheme(<RiskFactorsList factors={negativeFactors} />);
    ...
    it("handles single factor", () => {
      ...
      renderWithTheme(<RiskFactorsList factors={singleFactor} />);
    ...
    it("handles large number of factors", () => {
      ...
      renderWithTheme(<RiskFactorsList factors={manyFactors} maxItems={5} />);

      expect(
        screen.getByRole("button", { name: /show 15 more factors/i }),
      ).toBeInTheDocument();
    });
  });
});
