...
describe("ScoreHistoryChart", () => {
  ...
  describe("Loading State", () => {
    it("renders loading spinner when loading is true", () => {
      renderWithTheme(<ScoreHistoryChart data={[]} loading={true} />);
  ...
  describe("Error State", () => {
    it("renders error message when error prop is provided", () => {
      ...
      renderWithTheme(<ScoreHistoryChart data={[]} error={errorMessage} />);
  ...
  describe("Empty State", () => {
    it("renders info message when data is empty", () => {
      renderWithTheme(<ScoreHistoryChart data={[]} />);

      expect(
        screen.getByText(/no score history available/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Chart Display", () => {
    it("renders chart title", () => {
      renderWithTheme(<ScoreHistoryChart data={mockHistoryData} />);
  ...
  describe("Edge Cases", () => {
    it("handles single data point", () => {
      ...
      renderWithTheme(<ScoreHistoryChart data={singleDataPoint} />);
    ...
    it("handles all scores at minimum (0)", () => {
      ...
      renderWithTheme(<ScoreHistoryChart data={minScores} />);
    ...
    it("handles all scores at maximum (100)", () => {
      ...
      renderWithTheme(<ScoreHistoryChart data={maxScores} />);

      expect(screen.getByText("Score History")).toBeInTheDocument();
    });

    it("handles large dataset efficiently", () => {
      const largeDataset: CreditScoreHistory[] = Array.from(
        { length: 365 },
        (_, i) => ({
      ...
      renderWithTheme(<ScoreHistoryChart data={largeDataset} />);
