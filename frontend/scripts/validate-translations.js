...
function validateTranslations() {
  ...
  for (const lang of SUPPORTED_LANGUAGES) {
    ...
    if (emptyKeys.length > 0) {
      ...
      log(
        "red",
        `❌ ${lang.toUpperCase()}: Found ${emptyKeys.length} empty translations:`,
      );
  ...
  for (const lang of SUPPORTED_LANGUAGES) {
      ...
      for (const key in obj) {
        ...
        if (typeof obj[key] === "object" && obj[key] !== null) {
          ...
          if (basePlaceholders[fullKey]) {
            ...
            if (
              basePlaceholdersSet.size !== placeholdersSet.size ||
    ...
    if (inconsistentKeys.length > 0) {
      ...
      log(
        "red",
        `❌ ${lang.toUpperCase()}: Found ${inconsistentKeys.length} placeholder inconsistencies:`,
      );
      ...
      log(
        "green",
        `✅ ${lang.toUpperCase()}: Placeholder consistency verified`,
      );
    }
  }
  ...
  if (hasErrors) {
    ...
    log(
      "green",
      `🎉 Successfully validated ${SUPPORTED_LANGUAGES.length} languages with ${baseKeys.length} keys each.`,
    );
  }
}
